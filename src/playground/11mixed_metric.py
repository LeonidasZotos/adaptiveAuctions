...
class Auction:
    ...
    def get_end_of_auction_stats(self):
        ...
        mixed_metric = self.winner.get_native_valuation() * max_time_waited
        return self.inactivity_boost, self.revenue, max_time_waited, mixed_metric
...
def run_simulation(reserve):
    ...
    for auction_id in range(total_number_of_auctions):
        ...
        auction_modifier.update_bandit_valuations(auction.get_end_of_auction_stats()[
            0], auction.get_end_of_auction_stats()[3])  # We train on the mixed metrics, [3]
