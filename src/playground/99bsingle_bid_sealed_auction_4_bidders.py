"""In this file, 4 bidders participate in single-bid sealed auctions, with different loss aversions"""
import os
import numpy as np
import random
...
class Auction:
    def __init__(self, reserve_price, bidders):
        ...
        self.bidder3 = bidders[2]
        self.bidder4 = bidders[3]
    ...
    def run_auction(self):
        ...
        if self.bidder3.will_participate_in_auction(self.reserve_price):
            bid3 = self.bidder3.submit_bid()
            if bid3 > winning_bid:
                winning_bid = bid3
                auction_winner = self.bidder3
        if self.bidder4.will_participate_in_auction(self.reserve_price):
            bid4 = self.bidder4.submit_bid()
            if bid4 > winning_bid:
                winning_bid = bid4
                auction_winner = self.bidder4

        # All bidders that are not the winner lose the auction
        if auction_winner == self.bidder1:
            self.bidder2.lose_auction()
            self.bidder3.lose_auction()
            self.bidder4.lose_auction()
        if auction_winner == self.bidder2:
            self.bidder1.lose_auction()
            self.bidder3.lose_auction()
            self.bidder4.lose_auction()
        if auction_winner == self.bidder3:
            self.bidder1.lose_auction()
            self.bidder2.lose_auction()
            self.bidder4.lose_auction()
        if auction_winner == self.bidder4:
            self.bidder1.lose_auction()
            self.bidder2.lose_auction()
            self.bidder3.lose_auction()
...
def run_simulation(reserve):
    ...
    for auction in range(total_number_of_auctions):
        ...
        if (auction % number_of_auctions_per_set_of_bidders == 0):
            ...
            v_bidder3 = np.random.normal(mu_v, sigma_v)
            v_bidder4 = np.random.normal(mu_v, sigma_v)
            ...
            a_bidder3 = np.random.normal(mu_a, sigma_a)
            a_bidder4 = np.random.normal(mu_a, sigma_a)
            ...
            while v_bidder2 < valuations_min_max[0] or v_bidder2 > valuations_min_max[1]:
                v_bidder2 = np.random.normal(mu_v, sigma_v)
            while v_bidder3 < valuations_min_max[0] or v_bidder3 > valuations_min_max[1]:
                v_bidder3 = np.random.normal(mu_v, sigma_v)
            while v_bidder4 < valuations_min_max[0] or v_bidder4 > valuations_min_max[1]:
                v_bidder4 = np.random.normal(mu_v, sigma_v)
            ...
            while a_bidder2 < aversions_min_max[0] or a_bidder2 > aversions_min_max[1]:
                a_bidder2 = np.random.normal(mu_a, sigma_a)
            while a_bidder3 < aversions_min_max[0] or a_bidder3 > aversions_min_max[1]:
                a_bidder3 = np.random.normal(mu_a, sigma_a)
            while a_bidder4 < aversions_min_max[0] or a_bidder4 > aversions_min_max[1]:
                a_bidder4 = np.random.normal(mu_a, sigma_a)
            ...
            bidders = [Bidder(v_bidder1, a_bidder1, valuations_min_max, aversions_min_max, mu_a, sigma_a), Bidder(
                v_bidder2, a_bidder2, valuations_min_max, aversions_min_max, mu_a, sigma_a), Bidder(v_bidder3, a_bidder3, valuations_min_max, aversions_min_max, mu_a, sigma_a),
                Bidder(v_bidder4, a_bidder4, valuations_min_max, aversions_min_max, mu_a, sigma_a)]
