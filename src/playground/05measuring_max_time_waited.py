"""In this file, 04 is changed so that we measure the max_time_waited. The auction modofier still tries to maximise the revenue and changes 
the reserve price"""
import os
import numpy as np
import random
...
def run_simulation(reserve):
    ...
    for auction in range(total_number_of_auctions):
        ...
        auction_modifier.update_bandit_valuations(auction.get_end_of_auction_stats()[
            0], auction.get_end_of_auction_stats()[1])  # We train on revenue. Change to 2 if we want to train on max_time_waited
